...
def train_test(data, holdout_points):
    ...
    train = list(
        filter(lambda x: [x[0].m_x, x[0].m_s] not in holdout_points, data),
    )
    test = np.concatenate(
...
sectioned_data = [
    sig_arr[(sig_arr.m_x == pair[0])
            * (sig_arr.m_s == pair[1])] for pair in mxms
]
...
for array in non_test_data:
    tr, te = train_test_split(
        array[['m_jj', 'm_bbyy_mod', 'm_x', 'm_s', 'total_weight']].to_numpy(
        ).tolist(), test_size=0.3, random_state=0, shuffle=True,
    )
    train.append(tr)
    test.append(te)
